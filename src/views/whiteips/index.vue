  ...
  <div class="app-container">
    <el-form ref="formSearch" :inline="true" :model="formSearch" class="demo-form-inline">
      ...
      <el-form-item label="IP" prop="ip">
        <el-input v-model="formSearch.ip" placeholder="IP" />
      </el-form-item>
...
<script>
import { getWhiteipsList, whiteipActive, whiteipFrozen, getWhiteipsOfProviderList } from '@/api/whiteips'
...
export default {
  ...
  methods: {
    fetchData() {
      ...
      if (this.parentid) {
        getWhiteipsOfProviderList({ params: params, parentid: this.parentid }).then(response => {
        ...
        getWhiteipsList(params).then(response => {
          this.list = response.data.items || []
          this.listTotal = response.data.total || 0
          this.listLoading = false
        }).catch(error => {
    ...
    doActive(index, row) {
      ...
      this.listLoading = true
      whiteipActive(params).then(response => {
    ...
    doFrozen(index, row) {
      ...
      this.listLoading = true
      whiteipFrozen(params).then(response => {
        this.list[index] = response.data
        this.$message({
          message: '冻结成功！',
          type: 'success'
